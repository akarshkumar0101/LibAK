...
class GenerationalPerformance {
	...
	public GenerationalPerformance(NEAT neat) {
		this.populationSnapshot = new ArrayList<>(neat.getPopulation().size());
		this.speciesSnapshot = new ArrayList<>(neat.species.size());

		for (Genome geno : neat.getPopulation()) {
			// can add original geno because geno wont change (immutable)
			this.populationSnapshot.add(geno);
		}
